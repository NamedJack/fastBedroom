...
public class AddCartAnimation {
     ...
     */
    public static void AddToCart(final ImageView startView, View endView, Context mContext, final RelativeLayout rl, final double time) {
        ...
        final float xv = (float) (XtoX / time);
        ...
        final float g;
        if (xv>0) {
            g = (float) ((YtoY + xv * time) / time / time * 2);
        }else{
            g = (float) ((YtoY - xv * time) / time / time * 2);
        }
        ...
        va.setEvaluator(new TypeEvaluator<PointF>() {
            ...
            public PointF evaluate(float v, PointF pointF, PointF t1) {
                ...
                point.x = (float) (v * xv * time);
                if (xv>0) {
                    point.y = (float) (g * (v * time) * (v * time) / 2 - xv * v * time);
                }else {
                    point.y = (float) (g * (v * time) * (v * time) / 2 + xv * v * time);
